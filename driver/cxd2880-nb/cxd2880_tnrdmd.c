...
/*
 * G8YTZ narrowband experiment overrides. All default to "off" (stock
 * behaviour). Writable at runtime via /sys/module/cxd2880/parameters/,
 * read on every tune.
 *
 * nb_fs_hz : signal sample clock in Hz. Demod nominal rate is set to
 *            192 / Fs[MS/s] (clock mode A). 0 = off.
 * nb_reg4a : demod bandwidth code (8M=0,7M=2,6M=4,5M=6,1.7M=3). -1 = off.
 * nb_if_bw : tuner IF filter (5/6M=0,7M=1,8M=2,1.7M=3). -1 = off.
 */
unsigned int cxd2880_nb_fs_hz;
...
int cxd2880_nb_if_bw = -1;
module_param_named(nb_if_bw, cxd2880_nb_if_bw, int, 0644);
MODULE_PARM_DESC(nb_if_bw, "Override tuner IF filter code (-1=off)");

/*
 * Experimental (v2): hunting for a way past the 7-bit integer limit of the
 * T2 nominal-rate register.
 * nb_nomi_shift : divide the computed nominal rate by 2^shift before writing.
 * nb_reg15      : T2 only - override demod reg 0x15 (bank 0x04), the byte
 *                 after the nominal rate. Sony writes 0x03 at 1.7 MHz and
 *                 0x00 at 5-8 MHz. -1 = off.
 */
int cxd2880_nb_nomi_shift;
module_param_named(nb_nomi_shift, cxd2880_nb_nomi_shift, int, 0644);
MODULE_PARM_DESC(nb_nomi_shift, "Divide nominal rate by 2^n before writing (0=off)");

int cxd2880_nb_reg15 = -1;
module_param_named(nb_reg15, cxd2880_nb_reg15, int, 0644);
MODULE_PARM_DESC(nb_reg15, "Override byte after nominal rate: T2 0x15, DVB-T 0x65 (-1=off)");

/*
 * v3: DVB-T per-bandwidth registers (bank 0x04), which Sony sets alongside
 * the nominal rate. -1 = leave Sony's table value.
 * nb_t_gtdofst : reg 0x7d-0x7e (Sony mode A: 8M 0x0128 7M 0x124c 6M 0x1ff8 5M 0x265d)
 * nb_t_sst     : reg 0x71      (8M 0x35 7M 0x2f 6M 0x29 5M 0x24)
 * nb_t_notch   : reg 0x72-0x73 (8M 0xb300 7M 0xb800 6M 0xbeab 5M 0xc801)
 *                -2 = auto: (81920 / Fs[MS/s] - 28672) mod 65536, which
 *                reproduces all four of Sony's values from nb_fs_hz.
 * nb_t_notch2  : reg 0x6b-0x6c (8M 0x0102, others 0x0003)
 */
int cxd2880_nb_poke_reg = -1;
module_param_named(nb_poke_reg, cxd2880_nb_poke_reg, int, 0644);
int cxd2880_nb_poke_val;
module_param_named(nb_poke_val, cxd2880_nb_poke_val, int, 0644);

int cxd2880_nb_t_gtdofst = -1;
module_param_named(nb_t_gtdofst, cxd2880_nb_t_gtdofst, int, 0644);
int cxd2880_nb_t_sst = -1;
module_param_named(nb_t_sst, cxd2880_nb_t_sst, int, 0644);
int cxd2880_nb_t_notch = -1;
module_param_named(nb_t_notch, cxd2880_nb_t_notch, int, 0644);
int cxd2880_nb_t_notch2 = -1;
module_param_named(nb_t_notch2, cxd2880_nb_t_notch2, int, 0644);

static int nb_w16(struct cxd2880_tnrdmd *t, u8 reg, u16 v, const char *name)
{
	u8 d[2] = { v >> 8, v & 0xff };
	int ret = t->io->write_regs(t->io, CXD2880_IO_TGT_DMD, reg, d, 2);

	if (!ret)
		pr_info("cxd2880: NB %s=0x%04x\n", name, v);
	return ret;
}

int cxd2880_nb_override(struct cxd2880_tnrdmd *tnr_dmd, u8 nomi_reg)
{
	u64 nomi;
	u8 data[5];
	int ret;

	if (!cxd2880_nb_fs_hz && cxd2880_nb_reg4a < 0 && cxd2880_nb_reg15 < 0 &&
	    cxd2880_nb_t_gtdofst < 0 && cxd2880_nb_t_sst < 0 &&
	    cxd2880_nb_t_notch == -1 && cxd2880_nb_t_notch2 < 0 &&
	    cxd2880_nb_poke_reg < 0)
		return 0;

	if (tnr_dmd->clk_mode != CXD2880_TNRDMD_CLOCKMODE_A) {
		pr_err("cxd2880: NB override needs clock mode A\n");
		return -EINVAL;
	}

	ret = tnr_dmd->io->write_reg(tnr_dmd->io, CXD2880_IO_TGT_DMD,
				     0x00, 0x04);
	if (ret)
		return ret;

	if (cxd2880_nb_fs_hz) {
		nomi = div_u64(192000000ULL << 32, cxd2880_nb_fs_hz);
		if (cxd2880_nb_nomi_shift > 0 && cxd2880_nb_nomi_shift < 8)
			nomi >>= cxd2880_nb_nomi_shift;
		if ((nomi >> 32) > 0xff) {
			pr_err("cxd2880: NB fs %u Hz too low (min 750001)\n",
			       cxd2880_nb_fs_hz);
			return -EINVAL;
		}
		data[0] = (nomi >> 32) & 0xff;
		...
		data[3] = (nomi >> 8) & 0xff;
		data[4] = nomi & 0xff;
		ret = tnr_dmd->io->write_regs(tnr_dmd->io, CXD2880_IO_TGT_DMD,
					      nomi_reg, data, 5);
		if (ret)
			return ret;
		pr_info("cxd2880: NB fs=%u Hz nominal=%02x.%02x%02x%02x%02x reg 0x%02x\n",
			cxd2880_nb_fs_hz, data[0], data[1], data[2], data[3],
			data[4], nomi_reg);
	}

	if (cxd2880_nb_reg15 >= 0) {    /* byte after nominal: T2 0x15, DVB-T 0x65 */
		ret = tnr_dmd->io->write_reg(tnr_dmd->io, CXD2880_IO_TGT_DMD,
					     nomi_reg + 5, cxd2880_nb_reg15 & 0xff);
		if (ret)
			return ret;
		pr_info("cxd2880: NB reg%02x=0x%02x\n", nomi_reg + 5,
			cxd2880_nb_reg15 & 0xff);
	}

	if (cxd2880_nb_poke_reg >= 0) { /* raw bank-0x04 poke for exploration */
		ret = tnr_dmd->io->write_reg(tnr_dmd->io, CXD2880_IO_TGT_DMD,
					     cxd2880_nb_poke_reg & 0xff,
					     cxd2880_nb_poke_val & 0xff);
		if (ret)
			return ret;
		pr_info("cxd2880: NB poke 0x%02x=0x%02x\n",
			cxd2880_nb_poke_reg & 0xff, cxd2880_nb_poke_val & 0xff);
	}

	if (nomi_reg == 0x60) {         /* DVB-T only */
		if (cxd2880_nb_t_gtdofst >= 0) {
			ret = nb_w16(tnr_dmd, 0x7d, cxd2880_nb_t_gtdofst, "t_gtdofst");
			if (ret)
				return ret;
		}
		if (cxd2880_nb_t_sst >= 0) {
			ret = tnr_dmd->io->write_reg(tnr_dmd->io, CXD2880_IO_TGT_DMD,
						     0x71, cxd2880_nb_t_sst & 0xff);
			if (ret)
				return ret;
			pr_info("cxd2880: NB t_sst=0x%02x\n", cxd2880_nb_t_sst & 0xff);
		}
		if (cxd2880_nb_t_notch >= 0 ||
		    (cxd2880_nb_t_notch == -2 && cxd2880_nb_fs_hz)) {
			u16 n;

			if (cxd2880_nb_t_notch >= 0)
				n = cxd2880_nb_t_notch;
			else    /* 81920e6 / fs - 28672, mod 65536 */
				n = (u16)((s32)div_u64(81920000000ULL + cxd2880_nb_fs_hz / 2,
							cxd2880_nb_fs_hz) - 28672);
			ret = nb_w16(tnr_dmd, 0x72, n, "t_notch");
			if (ret)
				return ret;
		}
		if (cxd2880_nb_t_notch2 >= 0) {
			ret = nb_w16(tnr_dmd, 0x6b, cxd2880_nb_t_notch2, "t_notch2");
			if (ret)
				return ret;
		}
	}

	if (cxd2880_nb_reg4a >= 0) {
		ret = tnr_dmd->io->write_reg(tnr_dmd->io, CXD2880_IO_TGT_DMD,
					     0x4a, cxd2880_nb_reg4a & 0xff);
		if (ret)
			return ret;
		pr_info("cxd2880: NB reg4a=0x%02x\n", cxd2880_nb_reg4a & 0xff);
	}
		   ...
		   u8 is_cable, int shift_frequency_khz)
{
	...
	if (cxd2880_nb_if_bw >= 0) {
		data[4] = cxd2880_nb_if_bw & 0xff;
		pr_info("cxd2880: NB if_bw=0x%02x\n", data[4]);
	}
